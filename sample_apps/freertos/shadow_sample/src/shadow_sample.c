...
int main() {
	...
	char *JsonDocumentBuffer = osal_alloc(MAX_LENGTH_OF_UPDATE_JSON_BUFFER);
	size_t sizeOfJsonDocumentBuffer = MAX_LENGTH_OF_UPDATE_JSON_BUFFER;

	float temperature = 0.0;

	bool windowOpen = false;
	jsonStruct_t *windowActuator = osal_zalloc(sizeof(jsonStruct_t));
	...
	jsonStruct_t *temperatureHandler = osal_alloc(sizeof(jsonStruct_t));
	...
	ShadowInitParameters_t *sp = osal_zalloc(sizeof(ShadowInitParameters_t));
	...
	ShadowConnectParameters_t *scp = osal_zalloc(sizeof(ShadowConnectParameters_t));
